  ...
  (evil-save-goal-column
    (if (and evil-respect-visual-line-mode
        ...
        (let ((eol (save-excursion
          ...
          ;; beginning-of-visual-line should stop at the end of the line when
          ;; arg>1 and there is only one line in the buffer but it doesn't
          (when (and arg (> arg 1) (< (point) eol))
            (goto-char eol)))
      (let ((eol (save-excursion
                   (end-of-line)
                   (point))))
        (move-beginning-of-line arg)
        ;; Same problem as with beginning-of-visual-line
        (when (and arg (> arg 1) (< (point) eol))
          (goto-char eol))))))
