        ...
        (delq nil (mapcar #'(lambda (jump)
                              (let* ((mark (car jump))
                                     (pos (if (markerp mark)
                                            ...
                                            mark))
                                     (target-file (cadr jump)))
                                (when (and (stringp target-file)
                                           (not (file-remote-p target-file))
                                           (file-exists-p target-file)
                                           pos)
                                  (list pos target-file))))
    ...
    (let ((current-file-name (or (evil--jumps-current-file-name) (buffer-name)))
      ...
      (unless evil-jumps-cross-buffers
        ...
        (while (and (< idx size) (>= idx 0)
                    (let ((target-file (cadr (ring-ref target-list idx))))
                      (not (if (stringp target-file)
                               (string= current-file-name target-file)
                             (eq (current-buffer) target-file)))))
          (setq idx (+ idx shift))))
      (when (and (< idx size) (>= idx 0))
        ...
        (let* ((place (ring-ref target-list idx))
               (pos (car place))
               (target-file (cadr place)))
          (setq evil--jumps-jumping t)
          (if (stringp target-file)
              (unless (string= current-file-name target-file)
                (if (string-match-p evil--jumps-buffer-targets target-file)
                    (switch-to-buffer target-file)
                  (find-file target-file)))
            (unless (eq target-file (current-buffer))
              (display-buffer-same-window target-file nil)))
...
(defun evil--jumps-push ()
  ...
  (let* ((target-list (evil--jumps-get-window-jump-list))
         ...
         (current-file (or file-name (current-buffer)))
    ...
    (unless excluded
      (unless (ring-empty-p target-list)
        ...
        (setq first-file (car (cdr (ring-ref target-list 0)))))
      (unless (and (equal first-pos current-pos)
                   (or (equal first-file file-name)
                       (eq first-file (current-buffer))))
        (evil--jumps-message "pushing %s on %s" current-pos file-name)
        (ring-insert target-list `(,current-pos ,current-file))))
